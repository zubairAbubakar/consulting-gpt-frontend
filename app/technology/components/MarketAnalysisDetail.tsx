...
export const MarketAnalysisDetail: React.FC<MarketAnalysisDetailProps> = ({ item, axisName }) => {
  ...
  let scoreColor = 'text-gray-700 dark:text-slate-200';
  if (item.score >= 0.7) scoreColor = 'text-green-600 dark:text-green-400';
  else if (item.score >= 0.3 && item.score < 0.7)
    scoreColor = 'text-yellow-600 dark:text-yellow-400';
  else if (item.score < 0.3) scoreColor = 'text-red-600 dark:text-red-400';
  ...
  return (
    <div className="rounded-md border border-gray-200/80 bg-white p-4 shadow-sm dark:border-slate-700/70 dark:bg-slate-700/50">
      <h4 className="text-md mb-1 font-medium text-gray-700 dark:text-slate-200">
      ...
      <div className="mb-2 flex flex-wrap items-center gap-x-4 gap-y-1">
        ...
        <div className="flex flex-col text-sm text-gray-600 dark:text-slate-300">
          <span>Confidence: {confidencePercentage}%</span>
          <div className="mt-0.5 h-2 w-24 rounded-full bg-gray-200 dark:bg-slate-600">
            <div
      ...
      <p className="text-sm leading-relaxed text-gray-600 dark:text-slate-300">
        <span className="font-medium text-gray-700 dark:text-slate-200">Explanation:</span>{' '}
        {displayExplanation}
      </p>
      {item.explanation.length > EXPLANATION_TRUNCATE_LENGTH && (
        <Button
          ...
          className="mt-1 h-auto p-0 text-xs text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300"
        >
